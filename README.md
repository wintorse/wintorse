# wintorse

## Portfolio Website
**https://wintorse.vercel.app/**


--------------

## About wintorse
- Master's student at the University of Tokyo GSII
- Interested in data visualization, human-computer interaction, AR, AI, and IoT

## Works

<img width="1213" alt="assistfinder_teaser" src="https://github.com/wintorse/wintorse/assets/80797789/cb1b1317-15ad-40de-93b8-aa1048065c05" width="80%">

**AssistFinder**(2023)
- AssistFinder is a system designed to help visually impaired individuals understand their surroundings and identify suitable helpers by detecting and vocalizing the presence and status of store clerks.
  - (A) Device with RGB-D camera fixed to the cap.
  - (B) A person wearing the device and carrying a backpack containing a laptop.
  - (C) Output of YOLOv8 model, identifying a clerk and a customer and estimating the status of the clerk.
- As my Master's thesis, I conducted needs analysis, and implemented and evaluated a prototype system.

<br>
<br>
<img src="https://user-images.githubusercontent.com/80797789/223946898-59aa360d-aec7-468e-9c43-b6118cd24097.jpg" width="80%" />

**AgriAR**(2021)
- AR pilot system that visualize the current leaf amount(LAI) of your greenhouse plants and existence of pests, and show a desirable plant sample immersively in 3D
- Microsoft HoloLens 2, Unity
- As my Senior thesis, I conducted needs analysis, and implemented and evaluated a prototype system.
- This research was orally presented at [Japan Geoscience Union Meeting 2022(日本地球惑星科学連合2022年大会)](https://www.jpgu.org/meeting_j2022/) and [Visws2022(可視化情報学会第5回ビジュアリゼーションワークショップ)](https://www.vsj.jp/visws2022/).

<br>
<br>
<img alt="iii Exhibition 2022 official website" src="https://user-images.githubusercontent.com/80797789/223945429-d36bc409-7695-4c55-8b6d-35980df0f693.jpg" width="80%" />

**[iii Exhibition 2022 official website](https://archive.iiiexhibition.com/log/i3e24)**(2022)
- official website of a media art exhibition hosted by University of Tokyo III/GSII
- more info: https://www.iii.u-tokyo.ac.jp/research/221213iiiexhibition
- made with Nuxt.js
- created with seven other members as a team
- I played a central role in the implementation, and was also actively involved in the design(about 60-70% of the whole website design).

<br>
<br>
<img src="https://user-images.githubusercontent.com/80797789/224468269-d05ce70f-f12d-4cde-87b1-460e003a0041.jpg" width="80%" />

<img src="https://user-images.githubusercontent.com/80797789/225132632-113cf1b1-522e-4efd-8458-05901fa80aec.jpg" width="50%" />


**[一葉(Ichiyo)](https://archive.iiiexhibition.com/log/i3e24#work4)**(2022)
- media artwork made with four other members
- concept: fragile and transient interaction in neighborhood
- "A leaf engraved with a QR code extends the place of chance encounters into a digital space. What happens when we &apos;leaf&apos; a message?"
- 2D code on leaves: laser engraved
- Web app: made with Nuxt.js and firebase
- Web UI demo: https://youtu.be/hZ9n2qEj1pA
- exhibited as a part of iii Exhibition 2022
- featured in [美術手帖](https://bijutsutecho.com/magazine/news/exhibition/26372)
- got an Honorable Mention at [GOOD DESIGN NEW HOPE AWARD 2023](https://newhope.g-mark.org/award/2023/23NHA040011.html)
- accepted as [an academic article](https://doi.org/10.1145/3623509.3635866) of [ACM TEI 2024](https://tei.acm.org/2024/)
- awarded the [Best Prototype prize](https://drive.google.com/file/d/1qePISJrtg4K7D-NkwhhT_1OKxdbwSRYO/view?usp=sharing) at Student Design Competition session at TEI 2024


**[Portfolio website](https://wintorse.vercel.app/)**(2023)
- Portfolio website with playfulness
- created entirely by myself, from design to coding
- made with Next.js, Swiper, and Tailwind CSS
- customizable and playful UI which enables you to see the website as you like


**Koshizuka Lab. Logos**(2023)
- New logos for my lab at the university
- The logo renewal itself was my own initiative.
- I proposed several logo ideas and two were selected.
- These logos represent the initials of the lab name and the network, a common motif in the Internet, IoT, AI, smart city, and data exchange.
- I also designed a new presentation slide template for the lab.


<br>
<br>
<img src="https://user-images.githubusercontent.com/80797789/225132502-d3bf77c2-6b17-4cd6-bd97-7de1e166ecf9.jpg" width="55%" />


**[近見知新聞解読依頼](https://anothervision.tokyo/contents/details/chikamichi_shimbun)**(2019)
- a take-home riddles kit（持ち帰り謎） that is packed in a set of one copy that resembles an actual newspaper
- created with some members of [AnotherVision](https://anothervision.tokyo/)（東京大学謎解き制作集団AnotherVision）
- utilizes LINE Bot
- purchased by 4000+ people as of 2023
- on sale in [booth.pm](https://anothervision.booth.pm/items/1472887) as of Feb 2024

<br>
<br>

**[東大×ナゾトキ×ゲームブック　ハテナ王国の冒険 〜竜と魔法の書〜](https://book.froebel-kan.co.jp/book/detail/9784577049761)**(2020-2021)
- riddle-solving gamebook for children
- supervised by our team at [AnotherVision](https://anothervision.tokyo/) and published by Froebel-Kan Co.,Ltd
- I was engaged in making story structure and creating some riddles for children.


<br>
<br>

## GitHub Stats

[![wintorse's GitHub stats](github-readme-stats-theta-vert.vercel.app/api?username=wintorse&count_private=true&show_icons=true)](https://github.com/anuraghazra/github-readme-stats)

## Languages and Tools
[![My Skills](https://skillicons.dev/icons?i=html,css,react,nextjs,vuejs,nuxtjs,py,cpp,nodejs,kotlin,mysql,r,matlab,git,docker,firebase,unity,arduino,tensorflow,figma)](https://skillicons.dev)

<!--
**wintorse/wintorse** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
-->
